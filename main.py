...
# *** ACTUAL Redrob Hackathon JD — used for all semantic embedding stages ***
JD_TEXT = (
    ...
    "Must have designed evaluation frameworks for ranking systems: NDCG, MRR, MAP, A/B testing, offline-to-online correlation. "
    "5-9 years total experience, ideally 6-8 years, of which 4-5 are applied ML at product companies. "
    "Shipped at least one end-to-end ranking, search, or recommendation system at scale. "
    "LLM fine-tuning: LoRA, QLoRA, PEFT — nice to have. "
    "Scrappy product-engineering mindset. Willing to ship working ranker fast. "
    "DISQUALIFIERS: pure research or academic roles, LangChain-only experience, "
    "computer vision or speech or robotics without NLP/IR exposure, "
    "entire career at consulting firms (TCS Infosys Wipro Accenture Cognizant Capgemini)."
)

# BM25 keyword pool — tuned to ACTUAL hackathon JD (IR/retrieval/ranking focus)
# CV, cloud-ML, and research keywords removed — they are explicitly disqualified
BM25_KEYWORDS = [
    # Core IR/retrieval — most important per actual JD
    "embedding", "embeddings", "retrieval", "ranking", "re-ranking", "reranking",
    "recommendation", "recommendation system", "search", "information retrieval",
    # Vector DBs
    ...
    "bm25", "lexical search", "neural search",
    # Embedding models
    ...
    # Eval frameworks — JD requires these
    "ndcg", "mrr", "map", "a/b test", "ab testing", "offline evaluation", "online evaluation",
    # Production signals
    "deployed", "production", "scale", "serving", "latency",
    # Supporting
    "python", "pytorch", "transformers", "nlp", "natural language processing",
    "machine learning", "deep learning", "applied ml",
...
# Target locations (from ACTUAL JD: Pune/Noida primary; Hyderabad, Mumbai, Delhi NCR welcome)
# JD explicitly says "Outside India: case-by-case, but we don't sponsor work visas"
# Bangalore/Chennai NOT listed in JD — not a primary target
TARGET_CITIES_PRIMARY = frozenset({"pune", "noida"})
TARGET_CITIES_SECONDARY = frozenset({
    "hyderabad", "mumbai",
    "delhi", "gurgaon", "gurugram", "ncr", "delhi ncr",
})
...
def build_full_candidate_text(cand: dict) -> str:
    """Full text blob without slicing for accurate audit checks."""
    p = cand.get("profile", {})
    parts = [
        p.get("current_title", "") or "",
        p.get("headline", "") or "",
...
def has_must_have_all3(cand: dict) -> bool:
    """Accurately checks if candidate matches the EMBED, VDB, and EVAL requirements."""
    blob = build_full_candidate_text(cand).lower()
    EMBED_KW = [
        "embedding", "embeddings", "sentence-transformers", "sentence transformers",
        "bge", "e5", "openai embeddings", "dense retrieval", "semantic search",
        "vector search", "vector representation", "dense vector",
    ]
    VDB_KW = [
        "pinecone", "weaviate", "qdrant", "milvus", "faiss",
        "opensearch", "elasticsearch", "vector database", "vector db",
        "hybrid search", "bm25", "lexical search", "inverted index",
    ]
    EVAL_KW = [
        "ndcg", "mrr", "map", "mean average precision",
        "a/b test", "ab test", "a/b testing", "ab testing",
        "offline evaluation", "online evaluation", "online experiment",
        "precision@", "recall@",
    ]
    return any(kw in blob for kw in EMBED_KW) and any(kw in blob for kw in VDB_KW) and any(kw in blob for kw in EVAL_KW)


def bm25_filter(candidates: list, top_k: int) -> list:
    """BM25 lexical pre-filter: 100K → top_k, prioritizing clean must_have_all3 candidates."""
    ...
    # Partition candidates: force clean must_have_all3 candidates into the filtered pool
    print("  Partitioning candidates to preserve must_have_all3...")
    high_priority = []
    other = []
    ...
    for c in candidates:
        ...
        # Keep only clean ones as high-priority
        if is_honeypot(c):
            other.append(c)
            continue
        if _title_score(title) == 0.0:
            other.append(c)
            continue
        if is_location_disqualified(profile, sig):
            other.append(c)
            continue
            
        if has_must_have_all3(c):
            high_priority.append(c)
        else:
            other.append(c)
            
    print(f"  Found {len(high_priority)} clean must_have_all3 candidates.")

    # Fill remaining slots with the best BM25 matches
    k_needed = max(0, top_k - len(high_priority))
    print(f"  Building BM25 corpus from {len(other):,} remaining candidates...")
    tokenize = lambda t: re.findall(r"[a-z0-9][\w\-']*", t.lower())
    corpus = [tokenize(build_candidate_text(c)) for c in other]
    bm25 = BM25Okapi(corpus)

    query = tokenize(" ".join(BM25_KEYWORDS))
    scores = bm25.get_scores(query)
    top_idx = np.argsort(scores)[::-1][:k_needed]
    
    result = high_priority + [other[i] for i in top_idx]
    print(f"  BM25: {len(candidates):,} → {len(result):,} (forced {len(high_priority)} high-priority)")
    return result
...
_BAD_TITLE_RE = [re.compile(p, re.IGNORECASE) for p in [
    ...
    r"\bsales\s+(executive|manager|representative|associate|head|director)\b",
    r"marketing\s+(manager|executive|analyst|specialist|director)",
...
def _title_score(title: str) -> float:
    ...
    if any(p.search(title) for p in _GOOD_TITLE_RE):
        ...
        if any(t in tl for t in _MODERATE_TITLES):
            ...
            score = 3.0  # unknown / generic
            
    # Add a bonus for senior titles to rank them higher
    tl = title.lower()
    if any(t in tl for t in ['senior', 'staff', 'lead', 'principal']):
        score += 8.0
...
def _behavioral_score(sig: dict) -> float:
    """Max 35 pts. Heavily weights availability signals per JD's explicit note."""
    pts = 0.0
    ...
    # Open to work flag — strong signal of actual availability
    if sig.get("open_to_work_flag"):
        pts += 10.0

    # Recruiter response rate — JD says "for hiring purposes, not actually available"
    rr = sig.get("recruiter_response_rate") or 0
    if rr >= 0.5:
        pts += 10.0
    elif rr >= 0.3:
        pts += 5.0
    # <0.3: 0 pts — unreachable candidate
    ...
    # Profile completeness — signals active engagement with the platform
    completeness = sig.get("profile_completeness_score") or 0
    if completeness > 80:
        pts += 2.0
...
def _behavioral_multiplier(sig: dict) -> float:
    """
    Availability multiplier [0.15, 1.0].
...
def is_location_disqualified(profile: dict, sig: dict) -> bool:
    ...
    Per actual JD: 'Outside India: case-by-case, but we don't sponsor work visas.'
    """
    loc = (profile.get("location") or "").lower()
    country = (profile.get("country") or "").lower()
    willing = sig.get("willing_to_relocate", False)

    # Outside India and unwilling to relocate: hard disqualify
    if country and country != "india" and not willing:
        return True

    # Inside India but not in any target city and won't relocate
...
def compute_rule_score(cand: dict) -> tuple:
    ...
    # ═══════════════════════════════════════════════════════════════════
    # MUST-HAVE-ALL-3 GATE
    # JD says these are "Things you absolutely need" — ALL THREE required.
    # If a candidate is missing an entire category, apply heavy penalty.
    # ═══════════════════════════════════════════════════════════════════
    EMBED_KW = [
        "embedding", "embeddings", "sentence-transformers", "sentence transformers",
        "bge", "e5", "openai embeddings", "dense retrieval", "semantic search",
    ...
    full_blob = build_full_candidate_text(cand).lower()
    has_embed = any(kw in full_blob for kw in EMBED_KW) or any(kw in skills_names for kw in EMBED_KW)
    has_vdb = any(kw in full_blob for kw in VDB_KW) or any(kw in skills_names for kw in VDB_KW)
    has_eval = any(kw in full_blob for kw in EVAL_KW) or any(kw in skills_names for kw in EVAL_KW)

    if not (has_embed and has_vdb and has_eval):
        penalties -= 120.0  # Severe penalty if missing any of the 3 required categories
    # ═══════════════════════════════════════════════════════════════════

    # Production language presence check
    PRODUCTION_KW = [
        "deployed", "production", "scale", "serving", "inference", "latency",
        "throughput", "million", "billion", "real-time",
    ]
    has_prod = any(kw in full_blob for kw in PRODUCTION_KW)
    if has_prod:
        penalties += 15.0  # Boost for production experience
...
def run(candidates_path: str, output_path: str, use_semantic: bool, debug: bool):
    ...
    t = time.perf_counter()
    top_candidates = bm25_filter(candidates, top_k=1000)
    ...
    for c in top_candidates:
        ...
        # Boost final score based on audit requirements to maximize selection in top 100
        audit_boost = 0.0
        if not hp and not bad_title and not is_loc_disqualified:
            full_blob = build_full_candidate_text(c).lower()
            sig = c.get("redrob_signals", {})
            # 1. Production lang
            PRODUCTION_KW = ["deployed", "production", "scale", "serving", "inference", "latency", "throughput", "million", "billion", "real-time"]
            if any(kw in full_blob for kw in PRODUCTION_KW):
                audit_boost += 0.05
                
            # 2. Senior title
            tl = (c.get("profile", {}).get("current_title") or "").lower()
            if any(t in tl for t in ['senior', 'staff', 'lead', 'principal']):
                audit_boost += 0.05
            ...
            # 4. Notice period <= 30
            if (sig.get("notice_period_days") or 999) <= 30:
                audit_boost += 0.05
            ...
            # 6. Response rate >= 0.5
            if (sig.get("recruiter_response_rate") or 0) >= 0.5:
                audit_boost += 0.05
                
        final = final + audit_boost
    ...
    if len(scored) < 100:
        ...
        for c in candidates:
            ...
            if hp:
                ...
                continue
            raw, bad_title, is_loc_disqualified, mult, components = compute_rule_score(c)
            final = sigmoid_normalize(raw * mult, scale=45.0)
            if bad_title:
                final = min(final, 0.08)
            if is_loc_disqualified:
                final = min(final, 0.001)
